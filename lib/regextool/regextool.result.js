...
*/
	...
	return {
		...
		sanitize: function(text) {
			...
			if (text == '') {
				if (!template_cache['empty']) template_cache['empty'] = $('#template_match_string_empty').tmpl().wrap('<p>').parent().html();
				return template_cache['empty'];
			}

			// Value is undefined, use 'unset' template
			if (typeof text === "undefined") {
				if (!template_cache['unset']) template_cache['unset'] = $('#template_match_string_unset').tmpl().wrap('<p>').parent().html();
		...
		updateInfo: function() {
			$('#info-bar .total-matches').text(counter + ((counter == 1) ? "match" : "matches"));
		}
	};
})();