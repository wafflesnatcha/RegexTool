...
var RegexTool = (function() {
	...
	return {
		...
		restore: function() {
			console.log("pattern = %o", this.Storage.get('pattern'));
			if (this.Storage.get('pattern') === undefined) {
				console.info("first time visitor, restoring example data");
				this.Storage.set(this.config('example'));
			}
