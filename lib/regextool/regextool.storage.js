...
RegexTool.Storage = (function() {
	...
	return {
		...
		typecast: function(value) {
			...
			if ((typeof(value) === 'number' || typeof(value) === 'string') && value !== '' && !isNaN(value)) return parseFloat(value);
			return value;
		},
		...
		get: function(key) {
			...
			if (!key) return;
			return this.typecast(this._proxy.get(RegexTool.config('storage_prefix') + key));
		...
		saveElement: function(el) {
			...
			case 'input':
				switch (el.prop('type').toLowerCase()) {
					case 'checkbox':
					this.set(id, el.prop('checked') ? "1" : "0");
					break;
				default:
					this.set(id, el.val());
				}
